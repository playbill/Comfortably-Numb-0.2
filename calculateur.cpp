 ...
 * \brief Contient l'implementation de toutes les methodes de calcul de la classe Calculateur
 *
...
{
    pileC = Pile::donneInstance(); /// Initialisation de la pile associee au calculateur

    ///Initialisation du mode de la calculatrice : mode Degre et mode Complexe
...
{
    ///On verifie d'abord que la pile au moins deux elements
  ...
  {
         ///Si oui, on recupere les deux derniers elements de la pile
        Element* e1 = pileC->donneInstance()->pop();
        Element* e2 = pileC->donneInstance()->pop();
        ///On realise la multiplication
        Element& tmp = e1->operator *(*e2);
         ///On caste le resultat en fonction du mode la calculatrice (appel de la fonction Calulateur::cast())
        Element* res = (this->cast(&tmp));
        delete e1;
        delete e2;
        delete &tmp;
        ///On push le resultat sur la pile
        this->pileC->push(res);
...
Element* Calculateur::soustraction()
{
  ...
  else
  {
        ...
        Element* e1 = pileC->donneInstance()->pop();
        Element* e2 = pileC->donneInstance()->pop();
        ///On realise la soustraction
        Element& tmp = e2->operator -(*e1);
        ///On caste le resultat en fonction du mode la calculatrice (appel de la fonction Calculateur::cast())
        Element* res = (this->cast(&tmp));
        delete e1;
        delete e2;
        delete &tmp;
...
Element* Calculateur::fact()
{
    ...
    {   qDebug()<<"on est en entier";
        Entier* m = dynamic_cast<Entier*>(this->pileC->pop());
        int a,f;
...
Element* Calculateur::cos()
{
    ...
    ///On teste si la calculatrice est en mode radian ou degre
    if(isRadian())
    {
...
{
    ///On push 1/2 sur la pile
    Reel* e = new Reel(1./2.);
   this->pileC->push(e);
    ///On realise l'operation avant dernier element ^ dernier element (soit element ^ 1/2)
   return pow();
}
...
Element* Calculateur::sqr()
{
    ...
    if(typeid(*this->pileC->top()) == typeid(Complexe))
    {   /*! Pour les complexes : (a + bi)^2 = a^2 - b^2 + 2bai */
        qDebug()<<"Complexe sqr";
...
{
    ///On teste si le dernier element est une expression
    if(typeid(*this->pileC->top()) == typeid(Expression))
    {    throw std::logic_error("L'élément du huat de la pile est une expression");
    }
    else
    {
        ///Si ce n'en ai pas une, on recupere cet element
        Element* tmp = this->pileC->pop();
        ///On lui applique la fonction sign() (implementee dans toutes les sous classes de Constante)
        Element* e = tmp->sign();
        delete tmp;
        ///On push le resultat sur la pile
        this->pileC->push(e);
...
{
    ///On recupere les deux derniers elements de la pile
    Element* a = getPile()->pop();
    Element* b = getPile()->pop();
    ///On swap les elements correspondants dans la pile
...
Element* Calculateur::sum()
{
   Entier* e = dynamic_cast<Entier*>(this->pop());
    qDebug()<<"Appel de la fonction sum()";
